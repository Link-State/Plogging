...
function updateItemList() {
  ...
  let registItem = (item) => {
    ...
    itemImage.style.backgroundImage =
      "url(" + PATH + "/static/image/" + item + ".png" + ")";
...
function itemEquip(e) {
  ...
  let msg =
    EQUIPED[slot] === undefined || EQUIPED[slot] !== itemName ? "적용" : "해제";
  let answer = requestMessage("아이템을 " + msg + "하시겠습니까?");
  if (answer) {
    if (msg === "적용") {
      SOCKET.emit("request", {
        msg: "equipItem",
        data: { type: "equip", itemName: itemName },
      });
      if (EQUIPED[slot] !== undefined) {
        let originItem = document.getElementById(
          EQUIPED[slot] + "_itemEquipBtn"
        );
        originItem.innerHTML = "적용";
      }
      itemSlot.style.backgroundImage =
        "url(" + PATH + "/static/image/" + itemName + ".png" + ")";
      ...
      SOCKET.emit("request", {
        msg: "equipItem",
        data: { type: "unmountItem", itemName: itemName },
      });
      itemSlot.style.backgroundImage = "";
      itemSlot.value = "";
      delete EQUIPED[slot];
